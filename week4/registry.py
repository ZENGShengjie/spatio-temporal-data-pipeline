...
def get_trainer(name: str) -> BaseTrainer:
    ...
    if name == "arima":
        ...
        from models.stgcn_model import STGCNBaseTrainer
        return STGCNBaseTrainer()
    elif name == "agformer":
        from models.agformer_model import AGFormerTrainer
        return AGFormerTrainer()
    elif name == "stf":
        from models.stf_model import STFBaseTrainer
        return STFBaseTrainer()
    elif name == "agformer_static":
        from models.agformer_model import AGFormerStaticTrainer
        return AGFormerStaticTrainer()
    elif name == "stf_loc_only":
        from models.stf_model import STFLocOnlyTrainer
        return STFLocOnlyTrainer()
    else:
        raise ValueError(f"unknown model: {name}")

def list_models():
    return ["arima", "prophet", "lstm", "gru", "gcn", "gat",
            "stgcn", "agformer", "stf",
